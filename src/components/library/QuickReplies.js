...
const sameReply = (currentReply) => (reply) =>
  currentReply.value === reply.value
const diffReply = (currentReply) => (reply) =>
  currentReply.value !== reply.value
export function QuickReplies({
  ...
  const shouldComponentDisplay = useMemo(() => {
    ...
    const hasNext = !!nextMessage && !!nextMessage.messageID
  ...
  const handleSend = (repliesData) => () => {
    onQuickReply === null || onQuickReply === void 0
      ? void 0
      : onQuickReply(
  ...
  return (
    <View style={styles.container}>
      ...
      {replies.length > 0 && (
        ...
        >
          {(renderQuickReplySend === null || renderQuickReplySend === void 0
            ? void 0
            : renderQuickReplySend()) || (
