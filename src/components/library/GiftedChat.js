...
class GiftedChat extends React.Component {
  constructor(props) {
    ...
    // Prop IsKeyboardInternallyHandled false - handle keyboard on touch outside of component with KeyboardShift
    ...
    // REVIEW: Send Method is required for send data to server
    this.onSend = (messages = [], shouldResetInputToolbar = false) => {
      ...
      const newMessages = messages.map((message) => {
        return {
          ...
          datetime: new Date(),
          messageID:
            this.props.messageIdGenerator && this.props.messageIdGenerator(),
        }
      })
    ...
    // Method defined height of messages container and composer

    this.onContentSizeChange = (event) => {
      let newComposerHeight = event.nativeEvent.contentSize.height
      if (newComposerHeight > MAX_COMPOSER_HEIGHT) {
        // const newMessagesContainerHeight =
        //   this.getMessagesContainerHeightWithKeyboard(MAX_COMPOSER_HEIGHT)
        this.setState({
          composerHeight: MAX_COMPOSER_HEIGHT,
          // messagesContainerHeight: newMessagesContainerHeight,
        })
        return MAX_COMPOSER_HEIGHT
      } else {
        // const newMessagesContainerHeight =
        ...
        return newComposerHeight
      }
    }

    // Method change for onContentSizeChange - no use
  ...
  handleTextInputFocusWhenKeyboardHide() {
    ...
    if (!this._isTextInputWasFocused) {
      this._isTextInputWasFocused =
        ((_a = this.textInput) === null || _a === void 0
          ? void 0
          : _a.isFocused()) || false
    }
  }
  ...
  render() {
    if (this.state.isInitialized === true) {
      ...
      const actionSheet =
        ...
        (() => {
          var _a
          return (_a = this._actionSheetRef.current) === null || _a === void 0
            ? void 0
            : _a.getContext()
        })
...
// NOTE Modified backgroundColor scheme
